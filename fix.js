...
const headStart = indexHtml.match(/(<!DOCTYPE html>[\s\S]*?<\/style>)/)[1];
...
const scriptAndNavMatch = indexHtml.match(/(<script>[\s\S]*?<\/nav>)/);
const scriptAndNav = scriptAndNavMatch[1];
...
const restMatch = contactHtml.match(/<div class="split-text">[\s\S]*/);

if (restMatch && scriptAndNavMatch) {
    const newContent = headStart + contactCSS + '</head>\n<body>\n' + scriptAndNav + '\n' + bodyStart + restMatch[0];
    fs.writeFileSync('Contact.html', newContent, 'utf8');
    console.log('Restored successfully');
} else {
    console.log('Failed to match structure');
}
