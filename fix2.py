...
# Extract from <!DOCTYPE html> until /* 1. Hero Section */ in index.html
...
# Extract from <link rel="stylesheet" to </nav> from index.html
scriptAndNavMatch = re.search(r'(<link rel="stylesheet"[\s\S]*?<\/nav>)', indexHtml)
...
if restMatch:
    newContent = headCommon + contactCSS + scriptAndNav + '\n' + bodyStart + restMatch.group(0)
