...
headCommonMatch = re.search(r'(<!DOCTYPE html>[\s\S]*?\/\* 1\. Hero Section \*\/)', indexHtml)
headCommon = headCommonMatch.group(1).replace('/* 1. Hero Section */', '')
...
scriptAndNavMatch = re.search(r'(<\/style>[\s\S]*?<\/nav>)', indexHtml)
scriptAndNav = scriptAndNavMatch.group(1).replace('</style>', '', 1).strip()
...
# Search for the exact string of the TRUE event-inquiry-section down to the end
restMatch = re.search(r'(<section class="event-inquiry-section">\s*<div class="section-container split-layout reverse">\s*<div class="split-text">\s*<h2 class="section-title">Private Events & Buyouts<\/h2>[\s\S]*)', contactHtml)

if restMatch:
    newContent = headCommon + contactCSS + '\n' + scriptAndNav + '\n' + bodyStart + restMatch.group(1)
    with open('Contact.html', 'w', encoding='utf-8') as f:
        f.write(newContent)
    print("Restored successfully")
else:
    print("Failed to match TRUE section")
